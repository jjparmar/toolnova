...
export async function GET(request: Request) {
  ...
  if (format ==="ai") {
    return NextResponse.json({"@context":"https://schema.org","@type":"ItemList",
      ...
      itemListElement: filteredTools.map((tool, index) => ({"@type":"ListItem",
        position: index + 1,
...
function getToolFeatures(slug: string): string[] {
  const features: Record<string, string[]> = {"text-summarizer": ["Instant summarization","Multiple length options","Key points extraction","Multi-language support"],"paraphraser": ["Plagiarism-free output","Multiple modes","Context preservation","Synonym suggestions"],"grammar-fix": ["Grammar correction","Spelling check","Punctuation fix","Style improvement"],"essay-writer": ["Structured essays","Topic research","Citation support","Multiple formats"],"homework-solver": ["Step-by-step solutions","Multiple subjects","Detailed explanations","Instant results"],"flashcard-maker": ["Auto-generation","Multiple formats","Study modes","Export options"],"quiz-generator": ["Multiple question types","Customizable difficulty","Instant creation","Answer keys"],"merge-pdf": ["Multiple PDF support","Drag & drop reorder","Fast processing","No quality loss"],"split-pdf": ["Page selection","Range extraction","Batch processing","Instant download"],"image-compressor": ["Quality preservation","Multiple formats","Batch compression","Size reduction"],
  };
